...
module ManifoldGeometry = struct
  ...
  (** Fisher Information Metric g_ij(θ) for a Gaussian family.
      ...
      Given signature coefficients θ, we map them to (μ, σ²):
        μ = θ₁ (level-1 drift component)
        σ² = |θ₂..θ₁₄| (energy of higher-order terms)
      
      For Gaussian: g = diag(1/σ², 1/(2σ⁴))
      This gives the natural Riemannian metric on the space. *)
  ...
  (** Extract (μ, σ²) from log-signature coefficients.
      μ = level-1 component (drift direction)
      σ² = sum of squares of level-2+ components (path energy) *)
  ...
  (** Geodesic distance on the Gaussian Fisher manifold.
      ...
      For Gaussian family, the closed-form geodesic distance is:
      d(θ₁, θ₂) = √(2) · arccosh(1 + D/2) where D is the invariant.
      
      Simplified: we use the Rao distance for univariate Gaussians:
      d = √(2) · |ln(σ₂/σ₁)| + (μ₂-μ₁)²/(σ₁·σ₂) *)
  ...
  (** Exhaustion score: [0, 1].
      ...
      High score = approaching crash manifold while leaving expansion.
      Low score = firmly in expansion territory.
      
      Score = d(current, expansion) / (d(current, expansion) + d(current, crash)) *)
